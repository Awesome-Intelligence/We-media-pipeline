...
def remove_chatbot_sentences(text: str) -> tuple[str, list]:
    ...
    for line in text.splitlines(True):  # keepends=True
        ...
        if any(stripped.startswith(art) for art in artifact_lines):
            ...
            continue
        for art in PATTERNS["chatbot_artifacts"]:
            if stripped.startswith(art):
                changes.append(f'Removed "{art}" sentence')
                break
        else:
            new_lines.append(line)
...
def strip_markdown(text: str) -> tuple[str, list]:
    changes = []
    # Preserve all markdown elements: headers, bold, code blocks, etc.
    return text, changes
