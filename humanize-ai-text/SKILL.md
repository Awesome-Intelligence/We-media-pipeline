...
- Chatbot sentences (preserves 内容编辑丨/内容审核丨 lines)
...
### Header and Attribution Preservation

`transform.py` is configured to **preserve** structural markdown elements:
- `# 标题` / `## 二级标题` / `### 三级标题` — headers are kept intact
- `内容编辑丨xxx` / `内容审核丨xxx` — attribution lines are preserved
- Code blocks (` ``` `) are preserved

Only AI-signature content within these elements is rewritten (e.g. `serves as a` → `is a`), while the markup itself stays.

### Pitfalls

**The correct way to preserve lines in `remove_chatbot_sentences()`**

The original bug: using `continue` alone (skip pattern matching) without `new_lines.append(line)` caused attribution lines (`内容编辑丨`, `内容审核丨`) to be **silently deleted** — they never appeared in the output.

The fix: use `new_lines.append(line); continue` to preserve the line AND skip chatbot pattern matching:
```python
if any(stripped.startswith(art) for art in artifact_lines):
    new_lines.append(line)  # PRESERVE first
    continue                # then skip
```

The `for...else` pattern then correctly handles non-attribution lines: `break` skips the `else` (don't append); natural loop end triggers `else` (append the line).

**Sync between `patterns.json` and `remove_chatbot_sentences()`**

If you add new protected attribution prefixes:
1. Add them to `patterns.json` → `chatbot_artifacts` (for `detect.py` scanning)
2. Add them to the `artifact_lines` set inside `transform.py` → `remove_chatbot_sentences()` (for transform skip logic)

Both must stay in sync, or lines containing the new phrase will be silently deleted during transform.

**Cache invalidation**: After modifying `transform.py`, delete `__pycache__/transform.cpython-310.pyc` or restart Python, otherwise the old compiled `.pyc` runs instead of the new `.py`.
